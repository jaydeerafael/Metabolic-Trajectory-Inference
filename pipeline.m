...
filepath = '/Users/jaydee/Desktop/Rannís_proj/';
filename = fullfile(filepath, 'GSE123813_bcc_scRNA_counts.txt');
addpath(genpath('/Applications/CPLEX_Studio129/cplex/matlab/x86-64_osx/'));
addpath(genpath('/Users/jaydee/cobratoolbox'));
initCobraToolbox(false);
...
already_mapped_tag = 0;
optional_settings.func = {'biomass_maintenance', 'DM_atp_c_'};
optional_settings.not_medium_constrained = '0';

%% Load and Prepare Data
data_bcc = readtable(filename, 'Delimiter', '\t', 'ReadVariableNames', false);
data_bcc.Properties.VariableNames{1} = 'Gene_IDs';
sampleNames = arrayfun(@(x) sprintf('sample %d', x), 1:size(data_bcc, 2)-1, 'UniformOutput', false);
data_bcc.Properties.VariableNames(2:end) = sampleNames;
...
load(fullfile(filepath, 'cobratoolbo/test/verifiedTests/visualization/Recon3D_301/Recon3DModel_301.mat'));
model = Recon3DModel;
A_fast = fastcc_4_rfastcormics(Recon3DModel, epsilon, 0);
Cmodel_fast = removeRxns(Recon3DModel, Recon3DModel.rxns(setdiff(1:numel(Recon3DModel.rxns), A_fast)));
biomass_rxn = checkObjective(Cmodel_fast);
printRxnFormula(model, biomass_rxn);

%% Process Each Sample
for i = 1:size(matrix_data, 2)
    [model_out{i}, A_keep{i}] = fastcormics_RNAseq(Cmodel_fast, matrix_data(:,i), gene_ids, dico, ...
        biomass_rxn, already_mapped_tag, consensus_proportion, epsilon, optional_settings);
end
